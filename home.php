...
if (!empty($_POST)) {
    ...
    // Update LED state
    if (!empty($lednum) && !empty($ledstate)) {
        $sql_led = "UPDATE esp32_update_sensor SET " . $lednum . " = ? WHERE id = ?";
        $q_led = $pdo->prepare($sql_led);
        $q_led->execute(array($ledstate, $id));
    }

    // Update button state
    if (!empty($btnnum) && !empty($btnstate)) {
        $sql_btn = "UPDATE esp32_update_sensor SET " . $btnnum . " = ? WHERE id = ?";
        $q_btn = $pdo->prepare($sql_btn);
        $q_btn->execute(array($btnstate, $id));
    }
...
<body>
    <div class="topnav">
        <h3>- ARROSAGE CONTROLLING MONITORING -</h3>
    </div>
    ...
    <div class="content">
        <div class="cards">
            ...
            <div class="card">
                ...
                <label class="switch">
                    <input type="checkbox" id="ESP32_01_TogLED_01" onclick="GetTogBtnLEDState('ESP32_01_TogLED_01', 'LED_01')">
                    <div class="sliderTS"></div>
                </label>
                <h4 class="LEDColor"><i class="fas fa-lightbulb"></i> LED 2</h4>
                <label class="switch">
                    <input type="checkbox" id="ESP32_01_TogLED_02" onclick="GetTogBtnLEDState('ESP32_01_TogLED_02', 'LED_02')">
                    <div class="sliderTS"></div>
                </label>
                <!-- *********************************************************************** -->

                <!-- New Buttons for controlling the Buttons on ESP32 ********************** -->
                <h4 class="LEDColor"><i class="fas fa-toggle-on"></i> BUTTON 1</h4>
                <label class="switch">
                    <input type="checkbox" id="ESP32_01_Btn_01" onclick="GetBtnState('ESP32_01_Btn_01', 'Btn_01')">
                    <div class="sliderTS"></div>
                </label>
                <h4 class="LEDColor"><i class="fas fa-toggle-on"></i> BUTTON 2</h4>
                <label class="switch">
                    <input type="checkbox" id="ESP32_01_Btn_02" onclick="GetBtnState('ESP32_01_Btn_02', 'Btn_02')">
                    <div class="sliderTS"></div>
                </label>
                <!-- *********************************************************************** -->
            </div>
    ...
    <script>
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (myObj.id == "esp32_01") {
                        ...
                        if (myObj.Btn_01 == "ON") {
                            document.getElementById("ESP32_01_Btn_01").checked = true;
                        } else if (myObj.Btn_01 == "OFF") {
                            document.getElementById("ESP32_01_Btn_01").checked = false;
                        }
                        if (myObj.Btn_02 == "ON") {
                            document.getElementById("ESP32_01_Btn_02").checked = true;
                        } else if (myObj.Btn_02 == "OFF") {
                            document.getElementById("ESP32_01_Btn_02").checked = false;
                        }
                    }
                }
            };
        ...
        //------------------------------------------------------------
        function GetTogBtnLEDState(togbtnid, lednum) {
            var togbtnchecked = document.getElementById(togbtnid).checked;
            var togbtncheckedsend = togbtnchecked ? "ON" : "OFF";
            Update_LEDs_Btns("esp32_01", lednum, togbtncheckedsend, null, null);
        }
        //------------------------------------------------------------

        //------------------------------------------------------------
        function GetBtnState(btnid, btnnum) {
            var btnchecked = document.getElementById(btnid).checked;
            var btncheckedsend = btnchecked ? "ON" : "OFF";
            Update_LEDs_Btns("esp32_01", null, null, btnnum, btncheckedsend);
        }
        //------------------------------------------------------------

        //------------------------------------------------------------
        function Update_LEDs_Btns(id, lednum, ledstate, btnnum, btnstate) {
            if (window.XMLHttpRequest) {
                xmlhttp = new XMLHttpRequest();
            } else {
                xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
            ...
            xmlhttp.open("POST", "updateLed_btn.php", true);
            xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xmlhttp.send("id=" + id + "&lednum=" + lednum + "&ledstate=" + ledstate + "&btnnum=" + btnnum + "&btnstate=" + btnstate);
        }
        //------------------------------------------------------------
    </script>
</body>
